...
The rest of the viewer expects Omicron/Anfatec ``.txt`` headers that reference
separate channel files on disk.  To keep the existing GUI logic untouched we
translate Nanonis ``.sxm`` scans into a lightweight mirror directory that
contains:

* A synthesized header file with the usual ``xPixel``/``XScanRange`` fields.
* Per-channel data dumps (ASCII grids) for every forward/backward signal.

The conversion happens lazily when a folder is loaded.  Each Nanonis file gets a
stable cache directory inside ``<folder>/.sxmviewer_nanonis`` that we reuse as
long as the source file's size/mtime match.
"""
...
def parse_nanonis_spectroscopy(path: Path | str) -> List[Dict[str, object]]:
    ...
    try:
        ...
        return []
    bias_name, bias_data = _select_bias_axis(spec.signals)
    if bias_name is None or bias_data is None:
        return []
    bias = np.asarray(bias_data, dtype=float)
