...
from ...utils.units import _SI_BASE_UNITS
...
class MultiPreviewCanvas(FigureCanvas):
    ...
    def _convert_between_units(self, value, from_unit, to_unit):
        if value is None:
            return None
        if not from_unit or not to_unit:
            return value
        from_info = _SI_BASE_UNITS.get(from_unit)
        to_info = _SI_BASE_UNITS.get(to_unit)
        if not from_info or not to_info or from_info[0] != to_info[0]:
            return value
        try:
            return value * (from_info[1] / to_info[1])
        except Exception:
            return value

    def _convert_from_nm(self, value, target_unit):
        if value is None or target_unit is None:
            return value
        return self._convert_between_units(value, "nm", target_unit)

    def _convert_to_nm(self, value, source_unit):
        if value is None or source_unit is None:
            return value
        return self._convert_between_units(value, source_unit, "nm")

    def _extract_view_path(self, view):
        """Return the best candidate path for a view."""
        if not view:
            return None
        meta = view.get("meta") or {}
        return view.get("path") or meta.get("path") or meta.get("file_path")

    def _apply_source_path(self, target_view, source_view):
        path = self._extract_view_path(source_view)
        if not path:
            return
        target_view["path"] = path
        meta = target_view.setdefault("meta", {})
    ...
    def _axis_unit_for_view(self, view):
        if view:
            axis_unit = (view.get("axis_unit") or "").strip()
            if axis_unit:
                return axis_unit
        return self._guess_view_unit(view)
    ...
    def _on_crop_release(self, event):
        ...
        if x1 is not None and y1 is not None and self._crop_square:
            ...
            x1 = x0 + sx * side
            y1 = y0 + sy * side
        dx = abs(x1 - x0) if x1 is not None and x0 is not None else 0.0
        dy = abs(y1 - y0) if y1 is not None and y0 is not None else 0.0
        ...
        self._apply_source_path(new_view, view)
    ...
    def _register_crop_entry(self, view, bounds_data, pixel_bounds, square, update_size=True):
        ...
        real_unit = self._axis_unit_for_view(view)
        real_size = (0.0, 0.0)
        if bounds_data:
    ...
    def _render_history_entry(self, ax, entry, show_label=True):
        ...
        edge_color = '#ff66ff' if is_active else '#ffd166'
        line_width = 2.1 if is_active else 1.6
        ...
        axis_unit = entry.get("unit") or self._fixed_crop_template_unit or "nm"
        ...
        real_label = ""
        if any(real_size):
            real_w_nm = self._convert_to_nm(real_size[0], axis_unit)
            real_h_nm = self._convert_to_nm(real_size[1], axis_unit)
            real_label = f"{real_w_nm:.2f} nm × {real_h_nm:.2f} nm"
        text_lines = [f"#{seq}"]
        if real_label:
    ...
    def _clamp_template_dims(self, width_px, height_px, max_w, max_h, square=False):
        width = max(1, min(int(width_px), max_w))
        height = max(1, min(int(height_px), max_h))
        if square:
    ...
    def _compute_template_bounds_from_pixels(self, view, ax, width_px, height_px, square=False):
        ...
        if w == 0 or h == 0:
            return None
        width, height = self._clamp_template_dims(width_px, height_px, w, h, square=square)
        cx = w / 2.0
        cy = h / 2.0
    ...
    def _compute_pixels_for_real_size(self, view, ax, real_width, real_height, input_unit="nm"):
        if view is None or ax is None:
            return None
        arr_obj = view.get("arr")
        if arr_obj is None:
        ...
        denom_x = max(1, w - 1)
        denom_y = max(1, h - 1)
        axis_unit = self._axis_unit_for_view(view) or "nm"
        converted_width = self._convert_between_units(real_width, input_unit, axis_unit)
        converted_height = self._convert_between_units(real_height, input_unit, axis_unit)
        px_width = max(1, int(round((converted_width * denom_x) / max(1e-6, x_span))))
        px_height = max(1, int(round((converted_height * denom_y) / max(1e-6, y_span))))
    ...
    def _maybe_restore_manual_template(self):
        ...
        if dims.get("type") == "px":
            ...
            template = self._compute_template_bounds_from_pixels(view, ax, width, height, square=bool(dims.get("square")))
            if template:
            ...
            unit = dims.get("unit") or self._fixed_crop_template_unit or "nm"
            px_dims = self._compute_pixels_for_real_size(view, ax, width, height, input_unit=unit)
            if not px_dims:
                return False
            template = self._compute_template_bounds_from_pixels(
                view,
                ax,
                px_dims[0],
                px_dims[1],
                square=bool(dims.get("square")),
            )
            if template:
                bounds_data, pixel_bounds = template
                self._fixed_crop_template = {
    ...
    def set_fixed_crop_template_size(self, width, height, square=False):
        ...
        if view is None or ax is None:
            return False
        template = self._compute_template_bounds_from_pixels(view, ax, width, height, square=entry.get("square", False))
        if not template:
            return False
        bounds_data, pixel_bounds = template
        self._fixed_crop_template = {
    ...
    def get_fixed_crop_template_real_size(self):
        ...
        unit = self._fixed_crop_template_unit or "nm"
        real_w = abs(x1 - x0)
        real_h = abs(y1 - y0)
        real_w_nm = self._convert_to_nm(real_w, unit)
        real_h_nm = self._convert_to_nm(real_h, unit)
        return (real_w_nm, real_h_nm, "nm")
    ...
    def _render_template_overlay(self, ax, view):
        ...
        if self._fixed_crop_template_bounds:
            ...
            unit = self._fixed_crop_template_unit or "nm"
            real_dx = self._convert_to_nm(abs(bx1 - bx0), unit)
            real_dy = self._convert_to_nm(abs(by1 - by0), unit)
    ...
    def _apply_fixed_crop_quick(self, event, view, ax):
        ...
        if w == 0 or h == 0:
            return False
        width = min(max(1, int(template.get("width", 1))), w)
        height = min(max(1, int(template.get("height", 1))), h)
        if template.get("square"):
            size = min(max(width, height), min(w, h))
