...
def build_browse_context_page(viewer):
    ...
    layout.setSpacing(6)
    viewer.add_view_btn = QtWidgets.QPushButton("Add channel view")
    viewer.add_view_btn.setToolTip("Add the current channel as an extra preview")
    viewer.clear_views_btn = QtWidgets.QPushButton("Clear extra views")
...
def build_measure_context_page(viewer):
    ...
    layout.setContentsMargins(0, 0, 0, 0)
    layout.setSpacing(6)
    viewer.measure_profile_btn = QtWidgets.QPushButton("Measure profile")
    viewer.measure_profile_btn.setToolTip("Start or stop interactive profile measurement")
    viewer.measure_angle_btn = QtWidgets.QPushButton("Measure angle")
    viewer.measure_angle_btn.setToolTip("Start or stop angle measurement tool")
    viewer.exit_profile_btn = QtWidgets.QPushButton("Exit profile")
    viewer.exit_profile_btn.setToolTip("Exit the profile measurement mode")
    viewer.clear_profile_btn = QtWidgets.QPushButton("Clear profile")
    viewer.clear_profile_btn.setToolTip("Clear the current profile line and start fresh")
    viewer.show_profile_window_btn = QtWidgets.QPushButton("Show profile window")
...
def build_display_widget(viewer, parent):
    ...
    viewer.display_menu_btn.setText("Display ?")
